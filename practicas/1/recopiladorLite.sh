...
echo -e "País: $(echo "$WHOIS_IP" | grep -iE 'country:' | sed 's/[cC]ountry: \s*//')\n" | tee "$DOMAIN".txt
echo -e "Estado: $(echo "$WHOIS_IP" | grep -iE 'stateprov:' | sed 's/StateProv: \s*//')\n" | tee "$DOMAIN".txt
echo -e "Ciudad: $(echo "$WHOIS_IP" | grep -iE 'city:' | sed 's/City: \s*//')\n" | tee "$DOMAIN".txt
echo -e "Dirección: $(echo "$WHOIS_IP" | grep -iE 'address:' | sed 's/[aA]ddress: \s*//')\n" | tee "$DOMAIN".txt
...
echo -e "Teléfonos del Personal: $(echo "$WHOIS_IP" | grep -iE 'phone:' | sed 's/phone:\s*/TrabajadorPhone:/') | tee "$DOMAIN".txt
\n"
echo -e "\n<========== CONECTIVIDAD ==========>\n" | tee "$DOMAIN".txt
...
echo -e "Subdominios: $(dnsmap "$DOMAIN" | tail -n +5 | head -n -3)\n" | tee "$DOMAIN".txt
